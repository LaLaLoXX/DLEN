...
import cv2
from .LWN_arch import LWN
from .SEB_arch import SEB
...
class Illumination_Estimator(nn.Module):
    def __init__(
        ...
        #self.depth_conv = DeformConv2d(n_fea_middle, n_fea_middle, kernel_size=3, padding=1, bias=False)
        # Squeeze-and-Excitation (SE) 模块
        # CBAM 注意力模块
        # self.cbam = CBAM(n_fea_middle)

        # 激活函数
        self.wave = LWN(dim=n_fea_middle, wavelet='haar', initialize=True,use_ca=True,use_sa=True)
    ...
    def forward(self, img):
        ...
        # illu_fea = self.cbam(illu_fea)
        illu_fea = illu_fea + self.wave(illu_fea)
        illu_map = self.conv2(illu_fea)
        return illu_fea, illu_map



class MIAB(nn.Module):
    def __init__(
            self,
            dim,
            dim_head=64,
...
class ChannelMapper(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.reduce = nn.Conv2d(in_channels=in_channels, out_channels=16, kernel_size=1)
        self.expand = nn.Conv2d(in_channels=16, out_channels=out_channels, kernel_size=1)

    def forward(self, x):
        x = self.reduce(x)  # 降维
        x = self.expand(x)  # 调整到目标通道数
        return x
...
class IGAB(nn.Module):
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                MIAB(dim=dim, dim_head=dim_head, heads=heads),
    ...
    def forward(self, x, illu_fea):
        ...
        for (attn, ff) in self.blocks:
            ...
            # bhwc
            # x = lwn(x.permute(0,3,1,2)).permute(0,2,3,1) + x
            # bhwc
            x = ff(x) + x
...
class DLEN_Single_Stage(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, level=2, num_blocks=[1, 1, 1]):
        ...
        self.amplitude = SEB(inp_channels=in_channels, out_channels=out_channels, dim =n_feat)
    def forward(self, img):
        # img:        b,c=3,h,w
        ...
        output_img = self.denoiser(input_img,illu_fea) + self.amplitude(input_img)
...
class DLEN(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1,1,1]):
        ...
        modules_body = [DLEN_Single_Stage(in_channels=in_channels, out_channels=out_channels, n_feat=n_feat, level=2, num_blocks=num_blocks)
...
if __name__ == '__main__':
    ...
    model = DLEN(stage=1,n_feat=40,num_blocks=[1,2,2]).cuda()
    print(model)
